...
public class ApplicantController : ControllerBase
{
    ...
    [HttpPost("CreateApplicantInformation")]
    public async Task<IActionResult> CreateApplicantInformation([FromBody]CreateApplicantInformationCommand cmd, CancellationToken cancellationToken)
    {
        var response = await _requestDispatcher.Send(cmd, cancellationToken);
        return Ok(response.Value.Value);
    }

    [HttpPost("UpdateApplicantInformation")]
    public async Task<IActionResult> UpdateApplicantInformation([FromBody] UpdateApplicantCommand cmd, CancellationToken cancellationToken)
    {
        var response = await _requestDispatcher.Send(cmd, cancellationToken);
        return Ok(response.Value.Value);
    }
